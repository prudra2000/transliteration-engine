...
function insertTrie(root, roman, value) {
    let node = root;
...
// ── Trie builder ─────────────────────────────────────────────────────────────
function buildTrie(scheme) {
    const root = newNode();
    // Build a map from independent vowel cp → VowelPair for O(1) lookup
    const pairByIndependent = new Map(scheme.vowelPairs.map((p) => [p.independent, p]));
    // Consonants
...
// ── Engine ───────────────────────────────────────────────────────────────────
export function toGujarati(text, scheme) {
...
//# sourceMappingURL=toGujarati.js.map